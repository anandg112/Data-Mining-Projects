...
#                                                                              
#
as.real <- function(x){as.numeric(x)}                                                                              
rfversion=function() {                                                       
...
rulefit=function (x,y,wt=rep(1,n),cat.vars=NULL,not.used=NULL,
   ...
   read.file('rfout')
   save(thecall,file=paste(rfhome,'/thecall',sep='')) 
   invisible(getmodel())
}                                                                              
rfpred=function (x) {
...
rfexe=function(program,minimized=T) {
   ...
   if (platform=='windows') {
      if(minimized) { system('rf_go.exe',show.output.on.console=F,wait=T)}
      else {
      ...
      if(minimized) { system('./rf_go.exe > temp.out')}
      else { system('./rf_go.exe')}
   }
...
runstats=function(rfmod) {
   if(missing(rfmod)) {
      it=scan(paste(rfhome,'/rfout',sep=''),what='',quiet=T)
   }
   else { it=rfmod[[1]]}
